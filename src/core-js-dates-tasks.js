...
function getWeekNumberByDate(date) {
  ...
  if (!firstDay.getDay()) {
    ...
    daysFirstWeek = 8 - firstDay.getDay();
  }

  const days = (date - firstDay) / 1000 / 60 / 60 / 24 + 1;

  return days - daysFirstWeek <= 0
    ? 1
    : Math.ceil((days - daysFirstWeek) / 7) + 1;
}
...
function getNextFridayThe13th(date) {
  let result = new Date(date.getUTCFullYear(), date.getUTCMonth() + 1, 13);
  while (result.getUTCDay() !== 4) {
...
function getQuarter(date) {
  return Math.ceil((new Date(date).getUTCMonth() + 1) / 3);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    for (let i = 1; i <= countWorkDays + countOffDays; i += 1) {
      if (i <= countWorkDays && startDate <= endDate) {
        result.push(
          `${`0${startDate.getDate()}`.slice(-2)}-${`0${
